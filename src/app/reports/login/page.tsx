...
import { loginToReports, sendReportsOtp, verifyReportsOtp, resetReportsPassword } from '../actions';
...
export default function ReportsLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    const result = await loginToReports(formData);
  ...
  const handleSendOtp = async () => {
    ...
    setError('');
    setSuccessMsg('');
    const result = await sendReportsOtp();
    setLoading(false);
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    setError('');
    const result = await verifyReportsOtp(otp);
    setLoading(false);

    if (result.success) {
      setSuccessMsg('OTP Verified! Enter your new reports password.');
      setView('reset');
    } else {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    setError('');
    const result = await resetReportsPassword(newPassword);
    setLoading(false);

    if (result.success) {
      setSuccessMsg('Reports password changed successfully! You can now login.');
      setView('login');
      setPassword('');
      setOtp('');
      setNewPassword('');
  ...
  return (
    <div className={styles.container}>
      <div className={styles.loginCard}>
        ...
        {view === 'login' && (
          <>
            <h1 className={styles.title}>Secure Reports</h1>
            <p className={styles.subtitle}>Enter the master reports password to view financial data</p>
            ...
            <form onSubmit={handleLogin} className={styles.form}>
              ...
              <button type="submit" disabled={loading} className={styles.button}>
                {loading ? 'Authenticating...' : 'View Reports'}
              </button>
            </form>
        ...
        {view === 'reset' && (
          <>
            ...
            <p className={styles.subtitle}>Create a new reports password</p>
